...
class UpdateTaskReviewPriorityUseCaseTest {
    ...
    fun `Review MORE action updates task priority score`() {
        runBlocking {
            ...
            val currentTask = createTaskForTest(id = taskId, priorityScore = taskPriorityScore)
            val nextPriorityTask = createTaskForTest(id = 777L, priorityScore = nextPriorityScore)
