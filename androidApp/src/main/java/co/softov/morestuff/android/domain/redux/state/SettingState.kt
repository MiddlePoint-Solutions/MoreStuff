...
import co.softov.morestuff.android.domain.Defaults
...
sealed class SettingAction : Action.FeatureAction() {
    ...
    data class LoadSettings(val settings: UserSettings) : PriorityAction()
...
fun SettingState.reduce(action: Action): SettingState {
    return when (action) {
